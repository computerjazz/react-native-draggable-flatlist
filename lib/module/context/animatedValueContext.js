...
function useSetupAnimatedValues() {
  ...
  const touchPositionDiffConstrained = useDerivedValue(() => {
    ...
    const maxTranslatePositive =
      scrollViewSize.value - (activeCellOffset.value + activeCellSize.value); // Only constrain the touch position while the finger is on the screen. This allows the active cell
    ...
    const constrainedBase = isTouchActiveNative.value
      ...
      : touchPositionDiff.value; // Make sure item is constrained to the boundaries of the scrollview
